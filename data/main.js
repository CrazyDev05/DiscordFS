...
function navigate(e) {
  ...
  seek((width / totalDurationRect.width) * video.duration);
}
...
class Process {
  ...
  check(playbackRate, target, delay) {
    new Promise(resolve => setTimeout(resolve, delay))
        .then(() => {
          if (this.playbackRate === -1 || this.volume === -1)
            return;
          if (this.lastTime === video.currentTime) {
            reload().then(() => this.start(playbackRate, target, delay))
            return;
          }

          if (video.currentTime+0.5*playbackRate < target || !video.isConnected) {
            this.lastTime = video.currentTime;
            this.check(playbackRate, target, delay);
          } else {
            this.reset();
          }
        });
  ...
  start(playbackRate, target, delay) {
    video.volume = 0;
...
function handleTouchNavigate(e) {
  ...
  if (e.timeStamp - touchStartTime > 500) {
    ...
    seek((value / durationRect.width) * video.duration);
  }
}
